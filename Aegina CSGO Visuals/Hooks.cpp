...
static HRESULT __stdcall Hooks::hkEndScene(IDirect3DDevice9* pDevice){
	if (!init) {
		ImGui::CreateContext();
		ImGui::GetIO().Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\Arial.ttf", 20.f, 0, ImGui::GetIO().Fonts->GetGlyphRangesJapanese());
	...
	if (ShowMenu) {
		...
		if (ImGui::BeginTabItem("Other")) {
			...
			ImGui::Checkbox("Weapon Names", &Esp::Opt::Wpn::Name);
